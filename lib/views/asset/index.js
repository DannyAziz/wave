...
import {OpenSeaQueries, pickTextColorBasedOnBgColorSimple} from '@lib/utils/';
...
const Asset = props => {
  ...
  useEffect(() => {
    (async () => {
      if (images.length !== 0) {
        const cropRegion = {x: 0, y: 0, height: 30, width: 30};

        const path = await RNPhotoManipulator.crop(
          images[currentSlide],
          cropRegion,
        );
        ...
        console.log(result);
        const primaryColor = result.background;
        setStatusBarStyle(
  ...
  const handleScrollEnd = e => {
    ...
    if (nativeEvent && nativeEvent.contentOffset) {
      ...
      if (nativeEvent.contentOffset.x === 0) {
        ...
        currentSlide = parseInt(Math.ceil(approxCurrentSlide.toFixed(2)), 10);
      }
  ...
  return (
    <Styled.Container>
      ...
      <Back onPress={() => props.navigation.goBack()} style={statusBarStyle} />
      {assetBundle && (
        <Styled.ImageSlider
          horizontal
          pagingEnabled
      ...
      {!assetBundle && <Styled.Image source={{uri: images[0]}} />}
      <BottomSheet
        ref={bottomSheetRef}
        index={0}
